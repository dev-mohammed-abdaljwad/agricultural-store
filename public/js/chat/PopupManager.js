...
class PopupManager {
    ...
    constructor(config) {
        ...
        if (!this._dock) {
            ...
            console.log('[PopupManager] Created chat dock element');
        }
    ...
    async _loadMessages(convId) {
        ...
        try {
            // Use correct endpoint: GET /conversations/{id}
            ...
            container.innerHTML = '';
            // Response structure: data.conversation.messages (not data.messages)
            const msgs = (data.conversation?.messages || []).slice().reverse();
    ...
    _subscribePusher(convId) {
        if (typeof window.pusher === 'undefined') {
            console.warn('[PopupManager] Pusher not initialized, cannot subscribe to conversation channel.');
            return;
        }

        try {
            // Subscribe to the conversation channel via Pusher WebSocket
            // MessageSent event broadcasts on 'private-conversation.{conversationId}'
            const channel = window.pusher.subscribe(`private-conversation.${convId}`);
            
            // Listen for new messages via WebSocket event
            channel.bind('message.sent', (data) => {
                console.log('[PopupManager] Received message.sent event:', data);
                const isMine = data.sender_id == this.me;
                this._appendMsg(convId, data.message, isMine);
                if (!isMine) this._markRead(convId);
                // Update sidebar conversation order if needed
                // this._updateSidebarItem(convId, data.message, isMine);
            });

            // Handle subscription status
            channel.bind('pusher:subscription_succeeded', () => {
                console.log('[PopupManager] ✅ Subscribed to conversation channel:', convId);
            });

            channel.bind('pusher:subscription_error', (error) => {
                console.error('[PopupManager] ❌ Failed to subscribe to conversation channel:', error);
            });

            // Listen for typing indicator via Pusher whisper
            ...
            console.error('[PopupManager] Error subscribing to Pusher channel:', error);
        }
    }
    ...
    async send(convId) {
        ...
        try {
            // Use correct endpoint: POST /conversations/{id}/messages
            ...
            const data = await res.json();
            if (data.success) {
                // Append the message with correct sender
                const msg = data.message;
                this._appendMsg(convId, msg, true);
                this._updateSidebarItem(convId, msg, true);
            }
        } catch (err) {
