    ...
    <div class="w-full mx-auto px-2 sm:px-3 md:px-4 lg:px-6 xl:px-8 py-3 sm:py-4 md:py-6 flex-1 overflow-hidden flex flex-col">
        ...
        <div class="flex-1 overflow-hidden flex flex-col gap-2 sm:gap-3">
            ...
            <div class="bg-white rounded-lg sm:rounded-xl md:rounded-2xl border border-surface-200 shadow-sm md:shadow-md overflow-hidden flex flex-col flex-1">
                <!-- Messages Area -->
                <div class="flex-1 overflow-y-auto space-y-2 sm:space-y-3 md:space-y-4 pb-2 sm:pb-3 md:pb-4 px-2 sm:px-3 md:px-4 py-2 sm:py-3 md:py-4" id="messagesContainer">
            @forelse($conversation->messages as $message)
                <div class="flex {{ $message->sender_id === auth()->id() ? 'justify-end' : 'justify-start' }} group">
                    @if($message->sender_id === auth()->id())
                        ...
                        <div class="max-w-[70%] sm:max-w-[60%] md:max-w-sm lg:max-w-md">
                            <div class="bg-primary text-white rounded-lg sm:rounded-xl md:rounded-2xl md:rounded-tr-sm p-2 sm:p-3 md:p-4 shadow-sm">
                                @if($message->body)
                                ...
                                @if($message->attachment_url)
                                    <div class="mt-1 sm:mt-2 md:mt-3">
                                        @if($message->attachment_type === 'image')
                                            <img src="{{ $message->attachment_url }}" alt="Attachment" class="max-w-full rounded-md sm:rounded-lg max-h-40 sm:max-h-56">
                                        @else
                                            <a href="{{ $message->attachment_url }}" target="_blank" class="inline-flex items-center gap-1 text-xs sm:text-sm px-2 sm:px-3 py-1 sm:py-2 bg-white/20 hover:bg-white/30 rounded-md transition">
                                                <i class="material-symbols-outlined text-xs sm:text-sm">download</i>
                                ...
                                @if($message->attachment_url)
                                    <div class="mt-1 sm:mt-2 md:mt-3">
                                        @if($message->attachment_type === 'image')
                                            <img src="{{ $message->attachment_url }}" alt="Attachment" class="max-w-full rounded-md sm:rounded-lg max-h-40 sm:max-h-56">
                                        @else
                                            <a href="{{ $message->attachment_url }}" target="_blank" class="inline-flex items-center gap-1 text-xs sm:text-sm px-2 sm:px-3 py-1 sm:py-2 bg-surface-50 hover:bg-surface-100 rounded-md transition">
                                                <i class="material-symbols-outlined text-xs sm:text-sm">download</i>
                                                <span class="text-surface-700">{{ __('messages.download') }}</span>
                                            </a>
                                        @endif
                                    </div>
                                @endif
            ...
            <form id="messageForm" class="flex items-end gap-1 sm:gap-2 md:gap-3">
                @csrf

                <label class="cursor-pointer flex-shrink-0 p-2 sm:p-2.5 hover:bg-surface-100 rounded-full transition">
                    <input type="file" id="fileInput" class="hidden" accept="image/*,.pdf,.doc,.docx,.xls,.xlsx">
                    <i class="material-symbols-outlined text-base sm:text-lg text-surface-600">attach_file</i>
                </label>
                ...
                <div class="flex-1 relative min-w-0">
                    <input 
                        ...
                        class="w-full px-2 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm bg-surface-50 border border-surface-200 rounded-full focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary resize-none"
                    >
                </div>

                <button 
                    type="submit"
                    class="p-2 sm:p-2.5 md:p-3 bg-primary text-white rounded-full hover:bg-primary-dark active:scale-95 transition disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
                    id="sendBtn"
                >
                    <i class="material-symbols-outlined text-base sm:text-lg md:text-xl">send</i>
                </button>
            </form>

            <div id="filePreview" class="mt-1 sm:mt-2 md:mt-3 hidden">
                <p class="text-xs text-surface-500 flex items-center gap-1">
                    <i class="material-symbols-outlined text-xs">attach_file</i>
                    <span id="fileName" class="truncate"></span>
                </p>
            </div>

            <div id="loadingIndicator" class="mt-1 sm:mt-2 md:mt-3 hidden">
                <p class="text-xs text-surface-500 flex items-center gap-1">
...
<script>
    ...
    function addMessageToDOM(message) {
        ...
        if (message.attachment_url) {
            if (message.attachment_type === 'image') {
                attachmentHTML = `<img src="${message.attachment_url}" alt="Attachment" class="max-w-full rounded-md sm:rounded-lg max-h-40 sm:max-h-56 mt-1 sm:mt-2">`;
            } else {
                attachmentHTML = `<a href="${message.attachment_url}" target="_blank" class="inline-flex items-center gap-1 text-xs sm:text-sm px-2 sm:px-3 py-1 sm:py-2 bg-white/20 hover:bg-white/30 rounded-md transition mt-1 sm:mt-2">
                    <i class="material-symbols-outlined text-xs sm:text-sm">download</i>
                    <span>{{ __('messages.download') }}</span>
                </a>`;
            }
        ...
        const messageHTML = `
            <div class="flex ${isOwn ? 'justify-end' : 'justify-start'}">
                <div class="max-w-[70%] sm:max-w-[60%] md:max-w-sm lg:max-w-md ${isOwn ? 'bg-primary text-white' : 'bg-white border border-surface-200'} rounded-lg sm:rounded-xl md:rounded-2xl ${isOwn ? 'md:rounded-tr-sm' : 'md:rounded-tl-sm'} p-2 sm:p-3 md:p-4 shadow-sm">
                    ${message.body ? `<p class="text-xs sm:text-sm md:text-base leading-relaxed break-words">${message.body}</p>` : ''}
                    ${attachmentHTML}
                    <p class="text-xs ${isOwn ? 'text-white/70' : 'text-surface-400'} mt-1">${time}</p>
                </div>
            </div>
        `;
