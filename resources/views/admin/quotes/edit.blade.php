...
@section('title', 'تعديل عرض السعر #' . $quote->id . ' - نيل هارفست')
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-8">
            <div class="flex items-center gap-3 mb-4">
                <a href="{{ route('admin.quotes.show', $quote) }}" class="text-primary hover:text-primary-container">
                    <span class="material-symbols-outlined">arrow_back</span>
                </a>
                <h1 class="text-4xl font-black text-primary font-headline">تعديل عرض السعر #{{ $quote->id }}</h1>
            </div>
            <p class="text-on-surface-variant">الطلب رقم #{{ $quote->order->id }} - العميل: {{ $quote->order->customer->name }}</p>
        </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-outline-variant/10 p-6">
                ...
                <div class="space-y-6" id="items-container">
                    @foreach($quote->items as $index => $item)
                        <div class="border border-outline-variant/20 rounded-lg p-4 item-row" data-quantity="{{ $item->orderItem->quantity }}">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                                <!-- Product -->
                                <div>
                                    <label class="block text-sm font-bold mb-2">المنتج</label>
                                    <div class="bg-surface-container-low p-3 rounded-lg">
                                        <p class="font-bold text-on-surface">{{ $item->orderItem->product->name }}</p>
                                        <p class="text-xs text-on-surface-variant">الكمية: {{ $item->orderItem->quantity }} {{ $item->orderItem->product->unit }}</p>
                                    </div>
                            ...
                            <div class="bg-surface-container-low p-3 rounded-lg text-right">
                                ...
                                <p class="text-2xl font-black text-primary item-total">{{ number_format($item->total_price, 2) }} ج.م</p>
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="mt-6 bg-primary-fixed/10 border border-primary-fixed rounded-lg p-6 text-center">
                    ...
                    <p class="text-4xl font-black text-primary" id="grand-total">{{ number_format($quote->total_amount, 2) }} ج.م</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-outline-variant/10 p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="date" 
                               name="valid_until"
                               value="{{ $quote->expires_at->format('Y-m-d') }}"
                               required
                               class="w-full px-4 py-3 rounded-lg border border-outline-variant focus:ring-2 focus:ring-primary">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="notes" 
                                  ...
                                  class="w-full px-4 py-3 rounded-lg border border-outline-variant focus:ring-2 focus:ring-primary resize-none">{{ $quote->notes }}</textarea>
                    </div>
